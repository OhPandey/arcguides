...
import { getHeroFragments, getHeroStars, getShardCost, MAXPOINTS } from "../src/hero";
...
export default function TalentRenderer({ heroName, allowEdit, isRecommended, initSelectedTalentNodes }: Props) {
    ...
    const shards = getShardCost(talentPoints)
                ...
                >
                    <div className="relative w-225 h-160">
                        ...
                        <div className="absolute top-3 left-4 flex flex-col gap-2">
                            ...
                            {(stars > 0 || fragments > 0) && (
                                <p>Required Stars: {"⭐".repeat(stars)} {"✦".repeat(fragments)}<br /> Required Shards: {shards}</p>
                            )}
                        </div>
